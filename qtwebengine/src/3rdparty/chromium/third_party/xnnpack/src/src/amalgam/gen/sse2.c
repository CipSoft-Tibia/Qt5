...
void xnn_f32_vlrelu_ukernel__sse2_x8(
    ...
    const float* input,
    float* output,
    const union xnn_f32_lrelu_params params[ XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  assert(batch != 0);
  assert(batch % sizeof(float) == 0);
  assert(input != NULL);
...
void xnn_qc8_dwconv_minmax_fp32_ukernel_25p8c__sse2_mul16(
    ...
    size_t input_offset,
    const int8_t* zero,
    const union xnn_qc8_conv_minmax_params params[ XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  assert(channels != 0);
