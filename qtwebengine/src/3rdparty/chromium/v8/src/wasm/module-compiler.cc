...
void TransitiveTypeFeedbackProcessor::ProcessFunction(int func_index) {
  ...
  DCHECK_EQ(checked_feedback_length, call_direct_targets.size() * 2);
  FeedbackMaker fm(instance_, func_index, checked_feedback_length / 2);
  for (int i = 0; i < checked_feedback_length; i += 2) {
    ...
    if (IsWasmInternalFunction(value)) {
      ...
      DCHECK_LE(checked_polymorphic_length, 2 * kMaxPolymorphism);
