...
#endif
#if defined(_WIN64)
#define MAKE_SERVICE_REF(service) reinterpret_cast<void*>(&Target##service##64)
#else
#define MAKE_SERVICE_REF(service) reinterpret_cast<void*>(&Target##service)
#endif
...
#define INTERCEPT_NT(manager, service, id, num_params)        \
  (MAKE_SERVICE_REF(service) ? manager->ADD_NT_INTERCEPTION(  \
                                    service, id, num_params)  \
                              : false)
...
#define INTERCEPT_EAT(manager, dll, function, id, num_params)      \
  (MAKE_SERVICE_REF(function) ? manager->AddToPatchedFunctions(    \
                                    dll, #function,                \
                                    sandbox::INTERCEPTION_EAT,     \
                                    MAKE_SERVICE_NAME(function,    \
                                                      num_params), \
                                    id)                            \
                              : false)
#else  // !defined(SANDBOX_EXPORTS)
#define ADD_NT_INTERCEPTION(service, id, num_params)            \
  AddToPatchedFunctions(                                        \
      kNtdllName, #service, sandbox::INTERCEPTION_SERVICE_CALL, \
      MAKE_SERVICE_REF(service), id)
