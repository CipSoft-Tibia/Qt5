...
class tst_BaselineQuick3DPhysics : public QObject
{
    Q_OBJECT

public:
    tst_BaselineQuick3DPhysics();
...
tst_BaselineQuick3DPhysics::tst_BaselineQuick3DPhysics() : consecutiveErrors(0), aborted(false)
{
...
void tst_BaselineQuick3DPhysics::initTestCase()
{
...
void tst_BaselineQuick3DPhysics::cleanup()
{
...
void tst_BaselineQuick3DPhysics::testRendering()
{
    runTest();
}

void tst_BaselineQuick3DPhysics::setupTestSuite(const QByteArray &filter)
{
...
void tst_BaselineQuick3DPhysics::runTest(const QStringList &extraArgs)
{
...
bool tst_BaselineQuick3DPhysics::renderAndGrab(const QString &qmlFile, const QStringList &extraArgs, QImage *screenshot, QString *errMsg)
{
    ...
    QString cmd = QCoreApplication::applicationDirPath() + "/qquick3dphysics_qmlscenegrabber";
    ...
    QString tmpfile = usePipe
            ? QString("-")
            : QString("%1/qmlscenegrabber-%2-out.ppm").arg(QDir::tempPath()).arg(QCoreApplication::applicationPid());
...
quint16 tst_BaselineQuick3DPhysics::checksumFileOrDir(const QString &path)
{
    QFileInfo fi(path);
    ...
    if (fi.isDir()) {
        static const QStringList nameFilters = QStringList() << "*.qml"
                                                             << "*.cpp"
                                                             << "*.png"
                                                             << "*.jpg";
        quint16 cs = 0;
        const auto entryList = QDir(fi.filePath()).entryList(nameFilters, QDir::Dirs | QDir::Files | QDir::NoDotAndDotDot);
